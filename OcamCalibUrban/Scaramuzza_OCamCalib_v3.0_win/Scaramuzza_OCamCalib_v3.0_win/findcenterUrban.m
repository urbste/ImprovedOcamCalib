...
% [x0,~,exitflag,output] = fminunc(@errCenterUrban, x0, options, calib_data);
...
% sigma0q = 1^2;
% [rows,cols]=size(jacPP);    
% v = vPP;
% J = jacPP;
% Qll = sigma0q*eye(size(J,1),size(J,1));
% P0 = inv(Qll);
% % a posteriori variance
% calib_data.statPP.sg0 = (v'*P0*v) / (rows-cols);
% % empirical covariance matrix
% calib_data.statPP.Exx = calib_data.statPP.sg0 * inv(J'*P0*J);
% calib_data.statPP.varPP = diag(calib_data.statPP.Exx);        % variance of ext ori parameters
% calib_data.statPP.stdPP = sqrt(calib_data.statPP.varPP); % standard deviation of ext ori parameters
%% ================
