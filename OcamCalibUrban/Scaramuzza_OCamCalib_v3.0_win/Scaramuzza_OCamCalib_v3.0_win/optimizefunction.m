...
% Code was added to calculate the standard deviations of the exterior orientation
...
while (iter < max_iter && abs(MSE_new - MSE_old) > tol_MSE) && get(gcf,'CurrentChar') ~= 13
    ...
    %  added code , steffen urban
     ...
     % empirical covariance matrix
     calib_data.statIO.Exx = calib_data.statIO.sg0 * pinv(J'*P0*J);
